...
<ul><li> Used a closed-source LLM for ASR error correction post-processing, extracting the N-best list from the Zipformer-Transducer model, and wrote prompts combined with a biasing word list to send to GPT-4 for named entity correction. Offline experiments on a medical dataset showed that the <b>Rare word WER decreased from 37.8% to 17.5% </b>.</li>
...
<li> (Ongoing) Use SLAM-LLM to experiment with ASR/LLM integration for speech + text modality SFT, aiming to improve the consistency of ASR decoding results. The orthographic WER on the fully format WER, and the rare WER in achieved a better results compared with the production model, but the normilized WER still have room to improve. </li>
