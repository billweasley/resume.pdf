...
<li> (Ongoing) Use SLAM-LLM to experiment with ASR/LLM integration for speech + text modality SFT, aiming to improve the consistency of ASR decoding results. The Orthgoral WER on the fully format WER, and the rare WER in achieved a better results compared with the production model, but the normilized WER still have room to improve. </li>
